...
void testNewFunction() {
    std::cout << "Test new remove function\n";
    std::cout << "Stop test new remove function\n";
    SortedMultiMap smm = SortedMultiMap(asc);
    for(int i=0; i<20; i++){
        smm.add(i, i);
    }
    ...
    assert(it.getCurrent() == std::make_pair(0,0));
    it.remove();
    assert(it.getCurrent() == std::make_pair(1,1));

    // test for removing middle element
    for(int i=1; i<11; i++){
        it.next();
    }
    assert(it.getCurrent() == std::make_pair(11,11));
    it.remove();
    assert(it.getCurrent() == std::make_pair(12,12));

    // test for removing last element
    for(int i=12; i<19; i++){
        it.next();
    }
    assert(it.getCurrent() == std::make_pair(19,19));
    it.remove();
    assert(!it.valid());
